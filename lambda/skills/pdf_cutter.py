...
def main(params: tuple):
  ...
  # first get the id
  file_id = splited_message[4][1:]
  # second the pages
  page_from, page_to = int(splited_message[6]), int(splited_message[8])
  # get the file path
  file_path = f'lambdrive/documents/{file_id}.pdf'
  # generate a new hash
  new_hash = generate_hash(f"{file_id}_{splited_message[6]}_{splited_message[8]}")
